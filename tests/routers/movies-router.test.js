...
const userAuthorizationToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2MWFkYjRkMzc5MWE0NDQxNTcxNjcyMGQiLCJpYXQiOjE2Mzg3NzQwMjd9.GJjFvYkrEsyOsqH9ZVEeppiA7l-B-fa6hs7tKVcyzPI'
...
describe('Movies Controller', () => {
  ...
  //       const response = await supertest(app).get('/movies').set('Authorization', userAuthorizationToken).expect(200)
  ...
  //         const response = await supertest(app).get('/movies?limit=10&offset=20').set('Authorization', userAuthorizationToken).expect(200)
  ...
  //       const response = await supertest(app).get(`/movies/${existingMovie.movie_id}`).set('Authorization', userAuthorizationToken).expect(200)
  ...
  //       const response = await supertest(app).get(`/movies/${id}`).set('Authorization', userAuthorizationToken).expect(404)
  ...
  describe('POST /movies', function () {
    context('when this is a valid new movie', () => {
      it('should create a new movie object', async function () {
        ...
        const response = await supertest(app).post('/movies').set('Authorization', userAuthorizationToken).send(newMovieDetails).expect(201)
    ...
    //     const newMovieDetails = {}
    //     const response = await supertest(app).post('/movies').set('Authorization', userAuthorizationToken).send(newMovieDetails).expect(400)
  ...
  //         year: 2021,
  //       }
  //       const response = await supertest(app).put('/movies').set('Authorization', userAuthorizationToken).send(newMovieDetails).expect(201)
  ...
  //       const response = await supertest(app).put('/movies').set('Authorization', userAuthorizationToken).send(updatedMovieDetails).expect(200)
  ...
  //       const response = await supertest(app).patch('/movies/999').set('Authorization', userAuthorizationToken).send({}).expect(404)
  ...
  //         .set('Authorization', userAuthorizationToken)
  ...
  //       const response = await supertest(app).delete(`/movies/${idToDelete}`).set('Authorization', userAuthorizationToken).send({}).expect(404)
  ...
  //       const response = await supertest(app).delete(`/movies/${existingMovie.movie_id}`).set('Authorization', userAuthorizationToken).expect(200)
